...
addpath ('function');
% INPUTdir = 'NOISETC_SAMPLE2/DATA/datacache/';
% OUTPUTdir = 'NOISETC_SAMPLE2/DATA/datacache_prepro/';
INPUTdir = '/data/irma6/jrussel/YoungPacificORCA/TILTCOMP_NEW/ORCA_detrend/DATA/EVENT/';
OUTPUTdir = '/data/irma6/jrussel/YoungPacificORCA/TILTCOMP_NEW/ORCA_detrend/DATA/EVENT_preprocess/';
...
for ista = 1:length(stations)
    ...
    for ie = 1 : length(data_filenames) % begin file loop for each station
        ...
        display(data_filenames(ie).name);
        if ~isdir(inpath)
            continue
        end
